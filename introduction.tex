...
For some of the benchmarks, scores of the best models are exceeding 80 or even 90\%, for ANLI \citep{nie-etal-2020-adversarial}, however, the accuracy of even the best model does not exceed 70\%.
...
In a manual analysis, we find that, for the best performing models, some examples with incorrect predictions are in fact incorrect, but others concern questions on which humans may disagree too.
...
We find that accuracies (as computed on the majority label) is higher if the entropy of the human labels is low.
Next, we consider the distributional differences between model outputs and human labels, as measured by the Jensen Shannon Divergence (JSD) between the human label distributions and the models' softmax distributions over the possible answers.
We find that, the JSDs are lower than the ones reported for the previous generation of models by \citet{nie-etal-2020-learn} and are better than chance distributions. 
...
Interestingly, contrary to the findings of \citet{nie-etal-2020-learn}, we do observe a clear effect of scale: JSD shows a clear and steady decrease during training, and larger models have lower JSD than smaller models.
