...
\subsection{Model judgements in case of disagreement}
...
\subsection{Model versus human distributions}
...
Following \citet{nie-etal-2020-learn}, we consider Jensen-Shannon Divergence \citep[JSD][]{menendez1997jensen} to measure the distance between the two distributions.\footnote{Contrary to Kullback Leibner divergence, JSD is symmetric and bound between 0 and 1, making it more interpretable for our specific case.}
In \cref{fig:jsd_per_model}, we can see that, for MMLU, all considered models have probability distributions more similar to the human distributions than chance, but substantially more different than humans have among each other.\footnote{The human estimate was computed by \citet{nie-etal-2020-learn}, on an independent sample of human annotations on the same data.}
...
\paragraph{}
To further explore the role of scale in aligning with human distributions, we next study how alignment with human judgements develops during training.
In \cref{fig:jsd_training}, we show how the JSD of the three Llama architectures develops during training.
\dieuwke{TODO: include plots and discuss.}
